...
const abiSnapshotPath = resolve(join(root, 'packages/protocol/abi/envelope.abi.json'));
...
function checkCompatibility(prev: ABISnapshot, next: ABISnapshot) {
  ...
  for (const [ename, eprev] of Object.entries(prev.enums)) {
    ...
    for (let i = 0; i < eprev.length; i++) {
      ...
      if (!b || a.name !== b.name || a.value !== b.value) {
        problems.push(`Enum ${ename} changed at index ${i}: ${a.name}=${a.value} → ${b?.name}=${b?.value}`);
      }
    }
  }
  ...
  for (const [tname, fprev] of Object.entries(prev.tables)) {
    ...
    for (let i = 0; i < fprev.length; i++) {
      ...
      const b = fnext[i];
      if (!b || a !== b) {
        problems.push(`Table ${tname} field changed at index ${i}: ${a} → ${b}`);
      }
    }
  }
  ...
  for (const [uname, uprev] of Object.entries(prev.unions)) {
    ...
    for (let i = 0; i < uprev.length; i++) {
      ...
      const b = unext[i];
      if (!b || a !== b) {
        problems.push(`Union ${uname} entry changed at index ${i}: ${a} → ${b}`);
      }
    }
  }
...
function main() {
  ...
  if (existsSync(abiSnapshotPath)) {
    const abiPrev = JSON.parse(readFileSync(abiSnapshotPath, 'utf8')) as ABISnapshot;
