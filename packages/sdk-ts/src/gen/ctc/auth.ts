...
export class Auth implements flatbuffers.IUnpackableObject<AuthT> {
	...
	static getRootAsAuth(bb: flatbuffers.ByteBuffer, obj?: Auth): Auth {
		return (obj || new Auth()).__init(
			bb.readInt32(bb.position()) + bb.position(),
			bb,
		);
	}

	static getSizePrefixedRootAsAuth(
	...
	static addClientName(
		builder: flatbuffers.Builder,
		clientNameOffset: flatbuffers.Offset,
	) {
	...
	unpack(): AuthT {
		return new AuthT(this.jwt(), this.clientName(), this.protocol());
	}
...
export class AuthT implements flatbuffers.IGeneratedObject {
	...
	pack(builder: flatbuffers.Builder): flatbuffers.Offset {
		...
		const clientName =
			this.clientName !== null ? builder.createString(this.clientName!) : 0;

		return Auth.createAuth(builder, jwt, clientName, this.protocol);
	}
}
