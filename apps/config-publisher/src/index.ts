...
type InstrumentRow = {
  ...
  type: number;
  currency: string;
  tick_size_nanos: number;
  tick_size_ticks: number;
  lot_size_lots: number;
  meta: string | number;
};

type EtfComponentRow = {
  etf_instrument_id: number;
  component_instrument_id: number;
  weight_num: number;
  weight_den: number;
};
...
type RiskLimitRow = {
  team_id: number;
  instrument_id: number;
  pos_min_lots: string | number;
  pos_max_lots: string | number;
  notional_max_ticks: string | number;
...
async function getConfigVersion(client: Client): Promise<number> {
  const r = await client.query(
    "select (value->>'version')::bigint as v from settings where key='configVersion'",
  );
  if (r.rowCount && r.rows[0]?.v != null) return Number(r.rows[0].v);
  await client.query(
    "insert into settings(key, value) values ($1, $2) on conflict (key) do nothing",
    ["configVersion", { version: 0 } as any],
  );
  return 0;
}

async function bumpConfigVersion(client: Client, next: number) {
  await client.query(
    "insert into settings(key, value) values ($1, $2) on conflict (key) do update set value = excluded.value",
    ["configVersion", { version: next } as any],
  );
}
...
function buildSnapshot(
  ...
  const instOffsets = instruments.map((row) => {
    ...
    Instrument.addType(b, row.type as InstrType);
    ...
    Instrument.addMeta(b, BigInt(row.meta as any));
  ...
  const limitOffsets = limits.map((row) => {
    ...
    RiskLimit.addPosMinLots(b, BigInt(row.pos_min_lots as any));
    RiskLimit.addPosMaxLots(b, BigInt(row.pos_max_lots as any));
    RiskLimit.addNotionalMaxTicks(b, BigInt(row.notional_max_ticks as any));
...
async function publishOnce() {
  ...
  if (!ENGINE_CONTROL_URL) throw new Error("ENGINE_CONTROL_URL not set");

  await withPg(POSTGRES_URL, async (client) => {
    // Load rows
    const [inst, etf, opt, lim] = await Promise.all([
      client.query<InstrumentRow>(
        "select id, symbol, type, coalesce(currency,'USD') currency, tick_size_nanos, tick_size_ticks, lot_size_lots, meta from instruments order by id",
      ),
      client.query<EtfComponentRow>(
        "select etf_instrument_id, component_instrument_id, weight_num, weight_den from etf_components order by 1,2",
      ),
      client.query<OptionMetaRow>(
        "select instrument_id, underlying_instrument_id, strike_ticks, right, expiry, multiplier from options_meta order by instrument_id",
      ),
      client.query<RiskLimitRow>(
        "select team_id, instrument_id, pos_min_lots, pos_max_lots, notional_max_ticks, max_orders_per_sec from risk_limits order by 1,2",
      ),
    ]);
